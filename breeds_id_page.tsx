...
export default function BreedDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!breed) {
    return (
      <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#f0f0f0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "'Segoe UI', sans-serif" }}>
        <p style={{ fontSize: "48px", marginBottom: "16px" }}>🐕</p>
        <h1 style={{ fontSize: "24px", fontWeight: "900", marginBottom: "8px" }}>犬種が見つかりません</h1>
        <Link href="/breeds" style={{ color: "#f5a623", textDecoration: "none" }}>← 犬種一覧に戻る</Link>
      </div>
    );
  }

  const goodMatches = breed.goodMatch.map((mid: string) => BREEDS[mid]).filter(Boolean);
  const badMatches = breed.badMatch.map((mid: string) => BREEDS[mid]).filter(Boolean);
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "60px 24px 40px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            {goodMatches.map((m: any) => (
              <Link key={m.id} href={`/breeds/${m.id}`} target="_blank" style={{ textDecoration: "none" }}>
                <div
                  style={{ background: "#13131a", border: `1px solid ${m.color}55`, borderRadius: "12px", padding: "16px 20px", display: "flex", alignItems: "center", gap: "10px", transition: "all 0.2s" }}
        ...
        <div style={{ marginBottom: "40px" }}>
          ...
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            {badMatches.map((m: any) => (
              <Link key={m.id} href={`/breeds/${m.id}`} target="_blank" style={{ textDecoration: "none" }}>
                <div
                  style={{ background: "#13131a", border: "1px solid #e74c3c33", borderRadius: "12px", padding: "16px 20px", display: "flex", alignItems: "center", gap: "10px", transition: "all 0.2s" }}
        ...
        <div style={{ marginBottom: "60px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "12px" }}>
            {breed.variants.map((v: any, i: number) => (
